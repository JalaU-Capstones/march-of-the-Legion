...
/**
 * Unit tests for the {@link NorthPlacementStrategy} class.
 */
class NorthPlacementStrategyTest {

    private NorthPlacementStrategy strategy;

    @BeforeEach
    void setUp() {
        strategy = new NorthPlacementStrategy();
    }
    ...
    @Test
    void testPlaceUnitsInNorthDirection() throws SimulationException {
        // It should place units from south to north, row by row for each rank.
        ...
        verify(commander).setPosition(new Position(9, 0));
        verify(medic1).setPosition(new Position(8, 0));
        verify(medic2).setPosition(new Position(8, 1));
    }

    @Test
    void testNotEnoughRowsForUnitsThrowsException() {
        // It should throw a SimulationException if there are not enough rows for all ranks.
        List<ICharacter> units = new ArrayList<>();
